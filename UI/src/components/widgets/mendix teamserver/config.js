/**
 * MendixTeamServer widget configuration
 */
(function () {
    'use strict';
    angular
        .module(HygieiaConfig.module)
        .controller('MendixTeamServerWidgetConfigController', MendixTeamServerWidgetConfigController);
        MendixTeamServerWidgetConfigController.$inject = ['modalData', '$scope', 'collectorData', '$uibModalInstance'];
    function MendixTeamServerWidgetConfigController(modalData, $scope, collectorData, $uibModalInstance) {
        var ctrl = this,
        ...
        $scope.getJobs = function (filter) {
        	return collectorData.itemsByType('mendixteamserver', {"search": filter, "size": 20}).then(function (response){
        		return response;
        	});
        }
