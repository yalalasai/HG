...
(function () {
    ...
    angular
        ...
        .controller('MendixDeployWidgetConfigController', MendixDeployWidgetConfigController);
        MendixDeployWidgetConfigController.$inject = ['modalData', '$scope', 'collectorData', '$uibModalInstance'];
    function MendixDeployWidgetConfigController(modalData, $scope, collectorData, $uibModalInstance) {
        var ctrl = this,
        ...
        $scope.getJobs = function (filter) {
        	return collectorData.itemsByType('mendixdeploy', {"search": filter, "size": 20}).then(function (response){
        		return response;
        	});
        }
